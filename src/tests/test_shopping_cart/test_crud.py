...
def mock_validators(mocker):
    mocker.patch(f"{CRUD_MODULE}.validate_user", new_callable=AsyncMock, return_value=None)
    mocker.patch(f"{CRUD_MODULE}.validate_user_permission", new_callable=AsyncMock, return_value=None)
    mocker.patch(f"{CRUD_MODULE}.validate_movie_purchase_status", new_callable=AsyncMock, return_value=None)


@pytest.mark.asyncio
async def test_add_item_to_cart_success(
        db_session, test_user, test_movie, auth_user_schema, mocker, mock_validators
):
    mocker.patch(f"{CRUD_MODULE}.validate_movie", new_callable=AsyncMock, return_value=test_movie)
...
async def test_get_cart_creates_empty_if_not_exists(
        db_session, test_user, auth_user_schema, mock_validators
):

    initial_cart = await db_session.execute(select(Cart).where(Cart.user_id == test_user.id))
    ...
    final_cart = await db_session.execute(select(Cart).where(Cart.user_id == test_user.id))
    assert final_cart.scalar_one_or_none() is not None


@pytest.mark.asyncio
async def test_remove_cart_item(
        db_session, test_user, test_movie, auth_user_schema, mock_validators
):
    ...
    res = await db_session.execute(select(CartItem).where(CartItem.id == item.id))
...
async def test_clear_cart(
    ...
    items_res = await db_session.execute(select(CartItem).where(CartItem.cart_id == cart_id))
