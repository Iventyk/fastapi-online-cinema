...
async def validate_user_permission(
    ...
    has_permission = authenticated_user.permission == groups.MODERATOR.name or authenticated_user.permission == groups.ADMIN.name # noqa
    if authenticated_user.id != user_id and not has_permission:
        raise UserPermissionDenied("Not enough permission")


async def validate_movie(
        db: AsyncSession,
        movie_id: int
) -> Movie:
    movie_query = (
